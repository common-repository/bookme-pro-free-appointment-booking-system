  ...
  function getEventSelector(event) {
    ...
    function drawPopup($target, opt, div, day) {
        ...
        var $popup = $('<div class="text-center" style="width: 260px;">' +
            '<div class="checkbox bookme-pro-margin-bottom-md"><label>' +
            '<img src="' + opt.loadingImg + '" style="display: none; margin: 0 10px 0 -26px">' +
            '<input type="checkbox" class="bookme-pro-js-day-off" ' + ch + '>' +
            '<span class="bookme-pro-toggle-label">' + opt.we_are_not_working + '</span>' +
            '</label></div><div class="checkbox"><label>' +
            '<img src="' + opt.loadingImg + '" style="display: none; margin: 0 10px 0 -26px">' +
            '<input ' + di + ' type="checkbox" class="bookme-pro-js-repeat" ' + ch2 + '>' +
            '<span class="bookme-pro-toggle-label">' + opt.repeat  + '</span>' +
            '</label></div></div><hr><div class="text-right"><button type="button" class="btn btn-default">' + opt.close +
            '</button></div>');

        var $day_off = $popup.find('.bookme-pro-js-day-off');
        var $repeat  = $popup.find('.bookme-pro-js-repeat');
        ...
        $div
            .popover({
                html: true,
                placement: 'bottom',
                template: '<div class="popover bookme-pro-jcal" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
                content: function() {
                    return $popup;
                },
                trigger: 'manual'
        ...
        $('.bookme-pro-js-day-off, .bookme-pro-js-repeat', $popup).on('change', function () {
            var $this   = $(this);
    ...
    $('body').on('click', function (e) {
        var $target = $(e.target);
        if (!$target.hasClass('day') && $target.closest('.popover.bookme-pro-jcal').length == 0) {
            $('.popover.bookme-pro-jcal').popover('hide');
        }
    });
})(jQuery);