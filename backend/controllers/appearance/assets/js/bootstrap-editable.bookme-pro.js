jQuery(function($) {
    var BookmePro = function (options) {
        this.init('bookme_pro', options, BookmePro.defaults);
    };

    // Inherit from Abstract input.
    $.fn.editableutils.inherit(BookmePro, $.fn.editabletypes.abstractinput);

    $.extend(BookmePro.prototype, {
        ...
        value2html: function (values, element) {
            $.each(values, function (option_name, option_value) {
                ...
                $('.bookme-pro-js-option.' + option_name).each(function () {
                    var $this = $(this);
        ...
        value2input: function (values) {
            ...
            // Set codes.
            this.$tpl.closest('form').find('.bookme-pro-js-codes').html($(this.options.scope).data('codes'));
        },
    ...
    BookmePro.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
        tpl:       '<div/>',
        fieldType: 'input'
    });

    $.fn.editabletypes.bookme_pro = BookmePro;
    ...
    $.fn.editableform.template = '<form class="form-inline editableform"><div class="control-group"><div><div class="editable-input"></div><div class="editable-buttons"></div></div><div class="bookme-pro-js-codes"></div><div class="editable-error-block"></div></div></form>';
    $.fn.editableform.buttons = '<div class="btn-group btn-group-sm"><button type="submit" class="btn btn-primary editable-submit"><span class="glyphicon glyphicon-ok"></span></button><button type="button" class="btn btn-default editable-cancel"><span class="glyphicon glyphicon-remove"></span></button></div>';
});