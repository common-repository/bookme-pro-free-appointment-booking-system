<?php
namespace BookmePro\Backend\Controllers\Debug;

use BookmePro\Lib;

/**
 * Class Controller
 * @package BookmePro\Backend\Controllers\Debug
 */
class Controller extends Lib\Base\Controller
{
    const page_slug = 'bookme-pro-debug';
    ...
    public function index()
    {
        ...
        foreach ( apply_filters( 'bookme_pro_plugins', array() ) as $plugin ) {
    ...
    public function executeExportData()
    {
        ...
        header( 'Content-Disposition: attachment; filename=bookme_pro_db_export_' . date( 'YmdHis' ) . '.json' );
    ...
    public function executeImportData()
    {
        ...
        if ( $file = $_FILES['import']['name'] ) {
            ...
            if ( $json !== false) {
                ...
                header( 'Location: ' . admin_url( 'admin.php?page=bookme-pro-debug&status=imported' ) );
            }
        }

        header( 'Location: ' . admin_url( 'admin.php?page=bookme-pro-debug' ) );
