jQuery(function ($) {
    var Schedule = function ($container, options) {
        ...
        if (!$container.children().length) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('.bookme-pro-js-toggle-popover:not(.break-interval)', $container).popover({
                        html: true,
                        placement: 'top',
                        ...
                        $popover.find('.bookme-pro-popover-close').on('click', function () {
                    ...
                    $container
                    // Save Schedule
                        .on('click', '#bookme-pro-schedule-save', function (e) {
                            e.preventDefault();
                        ...
                        .on('click', '#bookme-pro-schedule-reset', function (e) {
                            e.preventDefault();
                            var ladda = Ladda.create(this);
                            ladda.start();
                            ...
                            $.ajax({
                                url: ajaxurl,
                                type: 'POST',
                                data: {action: 'bookme_pro_reset_breaks', breaks: $(this).data('default-breaks'), csrf_token: obj.options.l10n.csrf_token},
                                dataType: 'json',
                                success: function (response) {
                                    for (var k in response) {
                                        var $content = $(response[k]);
                                        $('[data-staff_schedule_item_id=' + k + '] .breaks', $container).html($content);
                                        $content.find('.bookme-pro-intervals-wrapper .delete-break').on('click', function () {
                        ...
                        .on('click', '.break-interval', function () {
                            var $button = $(this);
                            $('.popover').popover('hide');
                            var break_id = $button.closest('.bookme-pro-intervals-wrapper').data('break_id');
                            $(this).popover({
                                html: true,
                                placement: 'bottom',
                                template: '<div class="popover" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
                                content: function () {
                                    return $('.bookme-pro-js-content-break-' + break_id).html();
                                },
                                trigger: 'manual'
                            });
                        ...
                        .on('click', '.bookme-pro-js-save-break', function (e) {
                            var $table = $(this).closest('.bookme-pro-js-schedule-form'),
                                $row = $table.parents('.staff-schedule-item-row').first(),
                                $data = {
                                    action: 'bookme_pro_staff_schedule_handle_break',
                                ...
                                $break_interval_wrapper = $table.parents('.bookme-pro-intervals-wrapper').first(),
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.success) {
                                        ...
                                        obj.options.bookmeProAlert({error: [response.data.message]});
                                    }
                                },
                                complete: function () {
                                    ladda.stop();
                        ...
                        .on('click', '.bookme-pro-intervals-wrapper .delete-break', function () {
                            deleteBreak.call(this);
                        })

                        .on('change', '.break-start', function () {
                            var $start = $(this);
                            var $end = $start.parents('.bookme-pro-flexbox').find('.break-end');
                            hideInaccessibleBreaks($start, $end);
                        })

                        .on('change', '.working-schedule-start', function () {
                            var $this = $(this),
                                $end_select = $this.closest('.bookme-pro-flexbox').find('.working-schedule-end'),
                                start_time = $this.val();
                            ...
                            if (!$this.val()) {
                                $this.closest('.row').find('.bookme-pro-hide-on-off').hide();
                            } else {
                                $this.closest('.row').find('.bookme-pro-hide-on-off').show();
                            }
                        });
        ...
        function deleteBreak() {
            var $break_interval_wrapper = $(this).closest('.bookme-pro-intervals-wrapper');
            if (confirm(obj.options.l10n.are_you_sure)) {
                ...
                $.ajax({
                    url: ajaxurl,
                    type: 'POST',
                    data: {action: 'bookme_pro_delete_staff_schedule_break', id: $break_interval_wrapper.data('break_id'), csrf_token: obj.options.l10n.csrf_token},
                    success: function (response) {
                        if (response.success) {
    ...
    window.BookmeProStaffSchedule = Schedule;
});
