 ...
 * @package BookmePro\Frontend\Controllers\Booking\Lib
 */
class UserBookingData
{
    ...
    public function __construct( $form_id )
    {
        ...
        if ( $current_user && $current_user->ID ) {
            ...
            if ( $customer->loadBy( array( 'wp_user_id' => $current_user->ID ) ) ) {
                $this->set( 'full_name',  $customer->getFullName() );
                $this->set( 'first_name', $customer->getFirstName() );
                $this->set( 'last_name',  $customer->getLastName() );
                $this->set( 'email',      $customer->getEmail() );
                $this->set( 'phone',      $customer->getPhone() );
            } else {
        ...
        } elseif ( get_option( 'bookme_pro_cst_remember_in_cookie' ) && isset( $_COOKIE['bookme-pro-cst-full-name'] ) ) {
            $this->set( 'full_name',  $_COOKIE['bookme-pro-cst-full-name'] );
            $this->set( 'first_name', $_COOKIE['bookme-pro-cst-first-name'] );
            $this->set( 'last_name',  $_COOKIE['bookme-pro-cst-last-name'] );
            $this->set( 'email',      $_COOKIE['bookme-pro-cst-email'] );
            $this->set( 'phone',      $_COOKIE['bookme-pro-cst-phone'] );
        }
    ...
    public function addChainToCart()
    {
        ...
        if ( $this->get( 'repeated' ) ) {
            $series_uniq_id = microtime() . mt_rand( 0, PHP_INT_MAX );
        } else {
            $series_uniq_id = 0;
        }
    ...
    public function save( $payment = null )
    {
        ...
        foreach ( array( 'full_name', 'first_name', 'last_name', 'phone', 'email' ) as $field ) {
            if ( $this->get( $field ) != '' ) {
                ...
                $customer->setFields( array( $field => $this->get( $field ) ) );
            }
        }
        if ( get_option( 'bookme_pro_cst_create_account', 0 ) && ! $customer->getWpUserId() ) {
        ...
        if ( get_option( 'bookme_pro_cst_remember_in_cookie' ) ) {
            setcookie( 'bookme-pro-cst-full-name',  $customer->getFullName(),  time() + YEAR_IN_SECONDS );
            setcookie( 'bookme-pro-cst-first-name', $customer->getFirstName(),  time() + YEAR_IN_SECONDS );
            setcookie( 'bookme-pro-cst-last-name',  $customer->getLastName(),  time() + YEAR_IN_SECONDS );
            setcookie( 'bookme-pro-cst-phone',      $customer->getPhone(), time() + YEAR_IN_SECONDS );
            setcookie( 'bookme-pro-cst-email',      $customer->getEmail(), time() + YEAR_IN_SECONDS );
        }
