...
/**
 * Class Scheduler
 * @package BookmeProRecurringAppointments\Lib
 */
class Scheduler
{
    ...
    public function __construct( Lib\Chain $chain, $datetime, $until, $repeat, array $params, array $exclude )
    {
        ...
        $this->userData->set( 'days', array( 1, 2, 3, 4, 5, 6, 7 ) );

        foreach ( $exclude as $slots ) {
            $this->userData->set( 'slots', json_decode( $slots ) );
            $this->userData->addChainToCart();
            $this->userData->set( 'edit_cart_keys', array() );
        }
    ...
    public function build( array $slots )
    {
        ...
        foreach ( $this->userData->chain->getItems() as $item ) {
            $appointments_in_chain += $item->get( 'quantity' );
        }
    ...
    private function _addSlot( DatePoint $client_dp, $skip_days_off = false )
    {
        ...
        if ( $slot !== null ) {
            ...
            $this->userData->set( 'slots', json_decode( $slot['slots'] ) );
            $this->userData->addChainToCart();
            $this->userData->set( 'edit_cart_keys', array() );
