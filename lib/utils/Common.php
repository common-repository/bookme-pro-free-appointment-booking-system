<?php
namespace BookmePro\Lib\Utils;

use BookmePro\Lib;

/**
 * Class Common
 * @package BookmePro\Lib\Utils
 */
abstract class Common
{
    ...
    public static function getEmailHeaders($extra = array())
    {
        ...
        $headers[] = 'From: ' . get_option('bookme_pro_email_sender_name') . ' <' . get_option('bookme_pro_email_sender') . '>';
        if (isset ($extra['reply-to'])) {
            $headers[] = 'Reply-To: ' . $extra['reply-to']['name'] . ' <' . $extra['reply-to']['email'] . '>';
        }

        return apply_filters('bookme_pro_email_headers', $headers);
    }
    ...
    public static function optionToggle($option_name, $label = '', $help = '', array $options = array())
    {
        if (empty($options)) {
            $options = array(
                array(0, __('Disabled', 'bookme_pro')),
                array(1, __('Enabled', 'bookme_pro')),
            );
        }
    ...
    public static function optionFlags($option_name, array $options = array(), $label = null, $help = null)
    {
        ...
        echo self::getOptionTemplate($label, $option_name, $help, '<div class="bookme-pro-flags" id="' . $option_name . '">' . $control . '</div>');
    }
    ...
    {
        return apply_filters('wpml_translate_single_string', $original_value, 'bookme_pro', $name, $language_code);
    }
     ...
     * @return \stdClass[]
     */
    public static function getCustomFields($service_id = null)
    {
        $custom_fields = json_decode(get_option('bookme_pro_custom_fields'));
    ...
    {
        $custom_fields = Common::getCustomFields($service_id);
        foreach ($custom_fields as $key => $custom_field) {
            if ($service_id === null || in_array($service_id, $custom_field->services)) {
                switch ($custom_field->type) {
                    case 'textarea':
                    ...
                    case 'drop-down':
                        ...
                        foreach ($items as &$field) {
                            $field['label'] = self::getTranslatedString('custom_field_' . $custom_field->id . '_' . sanitize_title($custom_field->label) . '=' . sanitize_title($field['value']), $field['value'], $language_code);
                        }
                        $custom_field->label = self::getTranslatedString('custom_field_' . $custom_field->id . '_' . sanitize_title($custom_field->label), $custom_field->label, $language_code);
                        $custom_field->items = $items;
                        break;
     ...
     * @param string $title
     */
    public static function submitButton($id = 'bookme-pro-save', $class = '', $title = '')
    {
        printf(
            '<button%s type="submit" class="btn btn-lg btn-primary ladda-button%s" data-style="zoom-in" data-spinner-size="40"><span class="ladda-label">%s</span></button>',
            empty($id) ? null : ' id="' . $id . '"',
            empty($class) ? null : ' ' . $class,
            $title ?: __('Save', 'bookme_pro')
        );
    }
    ...
    {
        printf(
            '<button%s class="btn btn-lg btn-default%s" type="reset">' . __('Reset', 'bookme_pro') . '</button>',
            empty($id) ? null : ' id="' . $id . '"',
            empty($class) ? null : ' ' . $class
        );
    }
     ...
     */
    public static function deleteButton($id = 'bookme-pro-delete', $class = '', $modal = null)
    {
        printf(
            '<button type="button"%s class="btn btn-danger ladda-button%s" data-spinner-size="40" data-style="zoom-in"%s><span class="ladda-label"><i class="glyphicon glyphicon-trash"></i> ' . __('Delete', 'bookme_pro') . '</span></button>',
            empty($id) ? null : ' id="' . $id . '"',
            empty($class) ? null : ' ' . $class,
     ...
     * @param string $type
     */
    public static function customButton($id = null, $class = 'btn-primary', $title = null, array $attributes = array(), $type = 'button')
    {
        if (!empty($id)) {
            $attributes['id'] = $id;
        }
    ...
    public static function codes(array $codes, $flags = array())
    {
        ...
        echo '<table class="bookme-pro-codes"><tbody>' . $tbody . '</tbody></table>';
    }
    ...
    {
        if (self::$csrf === null) {
            self::$csrf = wp_create_nonce('bookme_pro');
        }
    ...
    public static function pageHeader($title)
    {
        ...
        <div class="page-header text-right clearfix">
            <div class="bookme-pro-page-title">
                <div class="bookme-pro-logo">
                    <img src="<?php echo plugins_url('backend/assets/images/logo-50.png', Lib\Plugin::getMainFile()) ?>">
                </div>
                <span><?php echo $title; ?></span>
            </div>
            <span class="bookme-pro-version"><?php echo __('Version', 'bookme_pro') . ' ' . Lib\Plugin::getVersion() ?></span>
        </div>
        <?php
    }
