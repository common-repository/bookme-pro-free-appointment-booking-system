...
class Staff extends Lib\Base\Entity
{
    ...
    protected static $table = 'bookme_pro_staff';
    ...
    public function save()
    {
        ...
        if ( $this->isLoaded() ) {
            ...
            do_action( 'wpml_register_single_string', 'bookme_pro', 'staff_' . $this->getId(), $this->getFullName() );
            do_action( 'wpml_register_single_string', 'bookme_pro', 'staff_' . $this->getId() . '_info', $this->getInfo() );
        }
        if ( $is_new ) {
            // Schedule items.
            $staff_id = $this->getId();
            foreach ( array( 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday' ) as $day_index => $week_day ) {
                ...
                $item->setStaffId( $staff_id )
                     ...
                     ->setStartTime( get_option( 'bookme_pro_bh_' . $week_day . '_start' ) ?: null )
                     ->setEndTime( get_option( 'bookme_pro_bh_' . $week_day . '_end' ) ?: null )
                     ->save();
            }
